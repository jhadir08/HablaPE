...
> **Resumen Ejecutivo:** HablaPE es una plataforma inteligente y multimodal que permite a los ciudadanos comprender sus derechos y deberes durante una intervención policial mediante orientación basada al 100% en normativa oficial del Estado peruano. Impulsado por **Gemma 4** como su cerebro central de orquestación, comprensión y generación, HablaPE transforma la complejidad jurídica en respuestas estructuradas, transparentes y libres de alucinaciones.
...
[ FastAPI Orchestrator Hub ] ◄─────────────────────────────────┐
     ...
     ├──► 3. RAG / ChromaDB (Búsqueda semántica + Filtros)     │ Contrato
...
* **Pipeline Offline (Construcción del Corpus):**
  ...
  4. *Almacenamiento Vectorial:* Indexación vectorial en **ChromaDB**.

* **Pipeline Online (Consulta):**
  1. *Búsqueda Semántica Filtrada:* El retriever consulta ChromaDB usando filtros por metadatos (ej. solo versiones vigentes).
...
│ • Retrieval en ChromaDB (Corpus Oficial) + Motor Determinístico (Validación en Python) │
└────────────────────────────────────────┬───────────────────────────────────────────────┘
                                         │
                                         ▼
┌────────────────────────────────────────────────────────────────────────────────────────┐
...
| **Normativa cambiante y derogaciones (Vigencia legal)** | Inclusión del metadato de `version` y `fecha_publicacion` en el índice de ChromaDB, validado mediante reglas determinísticas en Python. |
| **Consistencia en el pipeline entre capas** | Definición de un contrato JSON obligatorio emitido por Gemma 4 en la etapa de comprensión, actuando como pegamento estricto entre componentes. |
...
* **Modelo IA Central:** Gemma 4 (~12B Multimodal) vía SDK `google-genai` (Google AI Studio / Vertex AI).
* **Backend:** Python + FastAPI (Pydantic, Uvicorn).
* **Vector Store & RAG:** ChromaDB (embeddings compatibles con Google AI Studio).
